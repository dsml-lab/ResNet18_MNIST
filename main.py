...
#データローダー
dataloader1 = get_train_dataloader(root="data", batch_size=32)
dataloader2 = get_test_dataloader(root='data', batch_size=32)
# オプティマイザーの定義
...
def train(epochs):
    ...
    with open('./result/data10_train.csv', 'a') as f:
...
def val(epochs):
    ...
    with open('./result/data10_val.csv', 'a') as f:
...
def run():
    ...
    plt.savefig('./result/confusion_matrix_17.png')
    #lossグラフ作成
    ...
    plt.show()
    plt.savefig('./result/loss_graph_10.png')
     #accuracyグラフ作成
    fig, ax = plt.subplots(figsize=(8, 6))
    ...
    ax.legend(fontsize='20')
    plt.show()
    plt.savefig('./result/accuracy_graph_10.png')
    #間違えた数字の可視化
    ...
    for i,idx in enumerate(idx_list[:20]):
        ...
        plt.savefig('./result/misspreddata_10')
